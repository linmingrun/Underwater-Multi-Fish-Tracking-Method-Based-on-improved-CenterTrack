...
class CAGF(nn.Module):
    ...
    def forward(self, x1, x2):
        batch, c, h, w = x1.size()
        # 计算注意力
        q = self.query(x1).view(batch, -1, h*w).permute(0, 2, 1)
        k = self.key(x2).view(batch, -1, h*w)
        v = self.value(x2).view(batch, -1, h*w)
...
#         super(DynamicWeightLayer, self).__init__()
...
class EnhancedDynamicWeight(nn.Module):
    def __init__(self, channel, reduction=4):
        super().__init__()
        # 多分支卷积（修正ModuleList语法）
        self.conv_layers = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(channel, channel//reduction, kernel_size=ks, padding=ks//2),  # 1,3,5 卷积核
                nn.ReLU(),
                nn.Conv2d(channel//reduction, 1, 3, padding=1)
            )
            for ks in [1, 3, 5]  # 直接指定三种卷积核尺寸
        ])
    ...
    def forward(self, x):
        ...
        # 通道注意力
        b, c, _, _ = x.size()
        ca_weight = self.ca(self.avg_pool(x).view(b, c)).view(b, c, 1, 1)
        
        # 特征融合
        fused = self.fusion(torch.cat(multi_scale, dim=1))  # 拼接多尺度结果
        return fused * ca_weight  # 结合空间和通道注意力
...
class DLA(nn.Module):
    def __init__(self, levels, channels, num_classes=1000,
        ...
        self.level5 = Tree(levels[5], block, channels[4], channels[5], 2,
                           level_root=True, root_residual=residual_root)
        def get_factor(ch):
            """动态计算EMA的分组因子，确保通道数能被整除"""
            factor = 32  # 首选分组因子
            while factor > 0:
                if ch % factor == 0:  # 检查是否整除
                    return factor
                factor -= 1  # 递减尝试
            return 1  # 至少保证1组（相当于不使用分组）
        if opt.pre_img:
            self.pre_img_layer = nn.Sequential(
                nn.Conv2d(3, channels[0], kernel_size=7, stride=1, padding=3, bias=False),
                nn.BatchNorm2d(channels[0], momentum=BN_MOMENTUM),
                nn.ReLU(inplace=True))
            self.pre_img_se = EMA(channels[0], factor=get_factor(channels[0]))
            self.pre_img_weight = EnhancedDynamicWeight(channels[0])  
        ...
        if opt.pre_hm:
            self.pre_hm_layer = nn.Sequential(
                ...
                nn.BatchNorm2d(channels[0], momentum=BN_MOMENTUM),
                nn.ReLU(inplace=True))
            self.pre_hm_se = EMA(channels[0], factor=get_factor(channels[0]))
            self.pre_hm_weight = EnhancedDynamicWeight(channels[0])  
    ...
    def forward(self, x, pre_img=None, pre_hm=None):
        ...
        if pre_img is not None:
            pre_img_feat = self.pre_img_layer(pre_img)
            se_weights = self.pre_img_se(pre_img_feat)
            x = x + se_weights * pre_img_feat  
        
        if pre_hm is not None:
            pre_hm_feat = self.pre_hm_layer(pre_hm)
            dyn_weights = self.pre_hm_weight(pre_hm_feat)
            x = x + dyn_weights * pre_hm_feat  
